...
import { CategoryCreate, CategoryRealEstate, CategoryReturn } from "../interfaces";
...
const readCategoryRealState = async (categoryId: number): Promise<CategoryRealEstate> => {
   const categoryRealEstate: CategoryRealEstate | null = await categoryRepository.findOne({
      where: { id: categoryId },
